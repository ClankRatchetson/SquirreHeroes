...
const CATALOG: Readonly<Record<string, Card>> = { strike, flair: neutralCard, curse };

describe("generateRewardOffer", () => {
  it("exclut les malédictions du pool de récompenses", () => {
    const [offer] = generateRewardOffer(createRng(1), CATALOG, "casse_noix", "combat");
    expect(offer.cardChoices).not.toContain("curse");
  });

  it("applique le bon montant de Noisettes selon le type de nœud", () => {
    const [combatOffer] = generateRewardOffer(createRng(1), CATALOG, "casse_noix", "combat");
    const [eliteOffer] = generateRewardOffer(createRng(1), CATALOG, "casse_noix", "elite");
